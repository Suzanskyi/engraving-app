...
describe('Step2Customize Simplified State Management', () => {
  ...
  describe('Immediate Updates', () => {
    it('should immediately update text without debouncing', async () => {
      ...
      // Should generate composed image immediately (no debouncing)
      await waitFor(() => {
  ...
  describe('Simplified State Structure', () => {
    it('should consolidate text-related state updates', async () => {
      ...
      // Should generate final composed image with all changes
      await waitFor(() => {
    ...
    it('should handle text description preview generation', async () => {
      ...
      // Should use generateTextPreview for text descriptions
      await waitFor(() => {
        expect(ImageComposer.generateTextPreview).toHaveBeenCalledWith(
          expect.objectContaining({
  ...
  describe('Error Handling', () => {
    it('should handle image composition errors gracefully', async () => {
      ImageComposer.composeImage.mockRejectedValue(new Error('Composition failed'));
      ...
      expect(screen.getByPlaceholderText(/enter your custom text/i)).toBeInTheDocument();
    });

    it('should handle text preview generation errors gracefully', async () => {
      ImageComposer.generateTextPreview.mockRejectedValue(new Error('Preview failed'));
      ...
      await waitFor(() => {
        expect(ImageComposer.generateTextPreview).toHaveBeenCalled();
      });
